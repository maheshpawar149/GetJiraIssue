...
urlIP= 'https://scm.cls-group.com/wiki'; #URL for Confluence Server
...
def exithere(sheet):
	...
	data=""
	response = requests.post(urlIP+'/rest/api/content/20316171/child/attachment/att20316174/data', data=data,headers=headers, files=files)
...
def callENVStatusEXCEl():
	...
	response = requests.get(urlIP+'/download/attachments/20316171/CLSNow_Dev_Environment_Allocation_Status.xlsx',headers=headers);
...
def callENVReqEXCEL1():
	...
	##Getting Usage Sheet 
	headers={'Authorization':'Basic %s'% userpass}
	response = requests.get(urlIP+'/download/attachments/20316205/CLSNow_Dev_Environment_Usage.xlsx',headers=headers)
...
def saveENVReqExcel1(workbook2):
	...
	#data=[]
	response = requests.post(urlIP+'/rest/api/content/20316205/child/attachment/att20316206/data', data=data,headers=headers, files=files);
		...
		if( statAll=="REQUESTED" or statAll=="PENDING"):	
			...
			if (extention_on_req_present=="True"):
				...
				if (reqFrom=="fromExtend"):
					...
					print "ALOCATING NOW"
					allocateENV(memVal,teamVal,serverRequest,durVal,timezone,servers,"fromRequest");			
...
def allocateENV(memVal,teamVal,envVal,durVal,timezone,servers,ReqPrio):
	...
	if (memVal.isdigit())== True and (teamVal.isdigit())== True:
		...
		if(int(durVal) >int(durationLimit)):
			print "Error!!!Duration should not be greater than %s hours"%(durationLimit);	
		...
		if (durVal)==False:
			...
			if foundTemp!=1:
				...
				updateRequestPage(memVal,teamVal,serverRequest,durVal,start_Time,end_Time,timezone,"ALLOCATED",ReqPrio);
				
				exithere(sheet);	
			
def updateRequestPage(memVal,teamVal,serverRequest,durVal,start_Time,end_Time,timezone,ReqType,ReqPrio):
	#print ReqType;
...
def main():
	...
	try:
		memVal=(sys.argv[1].upper()).strip();	
		teamVal=(sys.argv[2].upper()).strip();	
		envVal=(sys.argv[3].upper()).strip();	
		durVal=sys.argv[4];
	...
	try:
		## For help parameter
		if (memVal=="DEFAULT" and teamVal=="DEFAULT" and envVal=="DEFAULT" and int(durVal)==0  and chagVal=="DEFAULT" and helpVal=="--HELP" and timezone=="DEFAULT"):
			helpDesc();
		...
		if (memVal!="DEFAULT" and teamVal!="DEFAULT"  and int(durVal)!=0  and chagVal=="DEFAULT" and helpVal=="DEFAULT" and timezone!="DEFAULT"):
			changeOpSuccess=0;						
			#Calling ENVALLOCATE	
			print "Calling updateRequestPage";
			#allocateENV(memVal,teamVal,envVal,durVal,timezone,servers);
			updateRequestPage(memVal,teamVal,envVal,durVal," "," ",timezone,"REQUESTED","fromMain");
		...
		###FOR STATUS Change of Env's	
		elif (memVal=="DEFAULT" and teamVal=="DEFAULT" and envVal!="DEFAULT" and int(durVal)==0  and chagVal!="DEFAULT" and helpVal=="DEFAULT" and timezone=="DEFAULT"):			
			...
			if acceptStatus not in ["O","o","M","m","R","r"]:
				print "ERROR!!!parameter for --change is incorrect.";
				exithere(sheet);
			
			print "********Changing Status of a Environment to MAINTENANCE/AVAILABLE/RELEASE********\n";
			###FOR STATUS to MAINTENANCE to AVAILABLE
			if acceptStatus in ["O","o"]:
				myLogger.info('------In MAINTENANCE->AVAILABLE Servers');				
				for innCount in range(1,len(servers)):
					serStatus=sheet.cell(row=innCount+1,column=4).value;
					if INserver==sheet.cell(row=innCount+1,column=2).value and serStatus in ["MAINTENANCE","Maintenance","maintenance"]:
						sheet.cell(row=innCount+1,column=4).value="AVAILABLE";
						print "DONE ...Status of %s Changed to AVAILABLE..."%(INserver);
						myLogger.info('------%s MAINTENANCE->AVAILABLE Changed'%(INserver));
						workbook1.save(filePath1);
						changeOpSuccess=1;
						
						exithere(sheet);
						print "Calling ReadRequest"
						readRequests("fromMaint",INserver);
						print "End ReadRequest"
						sys.exit();
						

			###FOR STATUS to AVAILABLE to MAINTENANCE
			elif acceptStatus in ["m","M"]:
				myLogger.info('------In AVAILABLE->MAINTENANCE Servers');		
				for innCount in range(1,len(servers)):
					serStatus=sheet.cell(row=innCount+1,column=4).value;
					if INserver==sheet.cell(row=innCount+1,column=2).value and serStatus in ["available","Available","AVAILABLE"]:
						sheet.cell(row=innCount+1,column=4).value="MAINTENANCE";
						print "DONE ...Status of %s Changed to MAINTENANCE..."%(INserver);
						myLogger.info('------%s AVAILABLE->INACIVE Changed'%(INserver));
						workbook1.save(filePath1);
						changeOpSuccess=1;
						exithere(sheet); 
			...
			###FOR STATUS to ALLOCATED to AVAILABLE / RELEASE
				...
				for innCount in range(1,len(servers)):
					...
					if INserver==sheet.cell(row=innCount+1,column=2).value and serStatus in ["ALLOCATED","Allocated","allocated"]:
						...
						updateRequestPage(memVal,teamVal,serverRequest,newDurval,start_Time,end_Time,"","COMPLETED","fromRel");
		...
		###FOR EXTENDING ALLOCATED DUARTION OF THE ENVIRONMENT		
		elif (memVal=="DEFAULT" and teamVal=="DEFAULT" and envVal!="DEFAULT" and int(durVal)!=0 and chagVal!="DEFAULT" and helpVal=="DEFAULT" and timezone=="DEFAULT"):
			acceptStatus=chagVal;INserver=envVal;INserver=INserver.upper();			
				...
				if (isreqFound=="ReqFOUND"):
					...
					for innCount in range(1,len(servers)):
						...
						if INserver==sheet.cell(row=innCount+2,column=2).value and serStatus in ["ALLOCATED","Allocated","allocated"]:
							...
							print end_Time;
							changeOpSuccess=1;
							updateRequestPage(memVal,teamVal,serverRequest,durVal,start_Time,end_Time,timezone,"EXTENDED","fromExtend");
							exithere(sheet);			
